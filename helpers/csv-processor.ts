...
export async function generateEmbeddingForRow(
  ...
  try {
    ...
    console.log(`Row ${row.rowNumber}: Generated embedding with ${validatedEmbedding.length} dimensions`);
    
    // Create unique ID for the vector
    const idContent = `${filename}_${row.rowNumber}_${row.question.substring(0, 50)}`;
...
export async function processCSVFile(
  ...
  try {
    ...
    console.log(`Successfully processed ${successfulRows} rows, failed ${failedRows} rows`);
